"""
Expose numpy's FFT functionality to numba by adding LLVM hooks to
call the internal ufunc FFT function pointers.
"""
...
class UFuncKey:
  """A key representing a ufunc and combination of argument types
  that should be used to call it"""

  name: str
  arg_types: Tuple[int, ...]


# Build a ctypes table holding fft ufunc objects and expose
# it as a global LLVM symbol, accessible via the LLVM code
# generated by the numba intrinsics below.
# This layer allows numba code that uses this FFT code
# to be cached.
...
def intrinsic_fft_factory(ufunc_name):
  ...
  def _invoke_fft_ufunc(typingctx, a, factor, out):
    ...
    def codegen(context, builder, signature, args):
      ...
      char_ptr_ptr_t = ir.PointerType(ir.PointerType(ir.IntType(8)))
      fn_llvm_type = ir.FunctionType(
        ir.VoidType(),
        [char_ptr_ptr_t, intp_ptr_t, intp_ptr_t, voidptr_t],
      )
      ...
      builder.call(
        fn_ptr,
        [
          builder.bitcast(args_ptr, char_ptr_ptr_t),
