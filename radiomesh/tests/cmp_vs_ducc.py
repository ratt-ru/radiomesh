...
def do_ducc0_wgridding(
  ...
  print(f'Time taken to map ({nrow},{nchan},{ncorr}) to ({ncorr},{npix},{npix}) = {time.time()-start}s')
...
if __name__ == "__main__":
    ...
    if args.backend == "ducc0":
      ...
      start = time.time()
      wgrid(uvw, vis, wgt, flags, freq, npix, npix, str(fov), 7, "[XX,XY,YX,YY]->[I,Q,U,V]")
      print(f'Time taken to map ({ntime},{nbl},{nchan},{ncorr}) to ({ncorr},{npix},{npix}) = {time.time()-start}s')
